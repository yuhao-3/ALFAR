#!/usr/bin/env python3
"""
Plot simplified three-way bucket analysis results for InfoSeek.
"""
...
def plot_threeway_bucket_analysis(dataset_name, stats_file, output_file):
    ...
    ax.set_xlabel('Performance-Based Bucket', fontsize=13, fontweight='bold')
    ...
    # Add key findings
    if "Corrective" in stats:
        corrective_stats = stats["Corrective"]
        delta = corrective_stats['delta_alfar_vs_regular']

        finding = f"Key Finding (Corrective Bucket):\n"
        finding += f"ALFAR +{delta:.2f}% over Regular MRAG\n"
        finding += f"ALFAR: {corrective_stats['alfar_acc']:.1f}% vs Regular MRAG: {corrective_stats['regular_mrag_acc']:.1f}%"
...
if __name__ == "__main__":
    # InfoSeek results
    dataset = "infoseek"
    stats_file = Path("results/bucket_analysis_threeway/infoseek_bucket_stats_simple.json")
